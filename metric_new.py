...
def entropy_calculation(x: Tensor):
    ...
    Args:
        x (torch.Tensor): A 1D probability distribution tensor.
    Returns:
...
def histogram2d(src: Tensor, dst: Tensor):
    """
    Compute a 2D histogram from PyTorch tensors.

    Args:
        src (torch.Tensor): Source tensor, assumed to be in the range [0, 1].
        dst (torch.Tensor): Target tensor, assumed to be in the range [0, 1].

    Returns:
        torch.Tensor: The 2D histogram.
    """
...
def calc_EN(fu: Tensor):
    """
    Compute the entropy of the fused image.

    Args:
        fu (torch.Tensor): Fused image tensor, assumed to be in the range [0, 1].
    ...
    # Compute the histogram; 256 bins correspond to a grayscale image in [0, 1]
